...
def main():
    ...
    selected_tastes = st.sidebar.multiselect("Select Tastes", unique_tastes, default=unique_tastes[:2])

    st.sidebar.header("User Preferred cuisine")
    unique_cuisine = df['user_cuisine'].unique()
    selected_cuisine = st.sidebar.multiselect("Select Cuisine", unique_cuisine, default=unique_cuisine[:2])
